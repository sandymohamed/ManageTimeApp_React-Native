...
export const AnalyticsScreen: React.FC = () => {
  ...
  // Enhanced date grouping function
  const groupProductivityData = (period: string, filteredTasks: Task[], start: Date, end: Date) => {
    const productivityData: { date: string; tasksCompleted: number; timeTracked: number }[] = [];

    switch (period) {
      case '7d':
        // Show all 7 days with abbreviated names
        for (let i = 6; i >= 0; i--) {
          const date = subDays(end, i);
          const dayStart = startOfDay(date);
          const dayEnd = endOfDay(date);
      ...
      case '30d':
        ...
        for (let i = 0; i < Math.min(weeks.length - 1, 4); i++) {
          const weekStart = weeks[i];
          const weekEnd = i === weeks.length - 2 ? end : weeks[i + 1];
          ...
          const weekTimeTracked = weekTasks.reduce((acc, task) => {
            const estimatedHours = task.metadata?.estimatedHours || 0;
            const actualHours = task.metadata?.actualHours || estimatedHours;
            return acc + actualHours;
          }, 0);
          ...
          productivityData.push({
            date: `W${i + 1}`, // W1, W2, etc.
            tasksCompleted: weekTasks.length,
            timeTracked: weekTimeTracked,
          });
        }
        break;

      case '90d':
        // Group by 2-week intervals (6 fortnights)
        for (let i = 5; i >= 0; i--) {
          const fortnightStart = subDays(end, (i + 1) * 15);
          const fortnightEnd = subDays(end, i * 15);

          const fortnightTasks = filteredTasks.filter(task => {
            if (!task.completedAt) return false;
            const completedDate = new Date(task.completedAt);
            return isWithinInterval(completedDate, { start: fortnightStart, end: fortnightEnd });
          });

          const fortnightTimeTracked = fortnightTasks.reduce((acc, task) => {
            const estimatedHours = task.metadata?.estimatedHours || 0;
            const actualHours = task.metadata?.actualHours || estimatedHours;
            return acc + actualHours;
          }, 0);
          ...
          productivityData.push({
            date: `${format(fortnightStart, 'MMM dd')}`,
            tasksCompleted: fortnightTasks.length,
            timeTracked: fortnightTimeTracked,
          });
        }
        break;
      ...
      case '1y':
        ...
        for (let i = 0; i < Math.min(months.length, 12); i++) {
          const monthStart = months[i];
          const monthEnd = i === months.length - 1 ? end : months[i + 1];
          ...
          const monthTimeTracked = monthTasks.reduce((acc, task) => {
            const estimatedHours = task.metadata?.estimatedHours || 0;
            const actualHours = task.metadata?.actualHours || estimatedHours;
            return acc + actualHours;
          }, 0);
  ...
  const calculateAnalytics = useMemo(() => {
    ...
    // Enhanced productivity data with smart grouping
    const dailyProductivity = groupProductivityData(selectedPeriod, filteredTasks, start, end);

    // Weekly trends
    const weeklyTrends: { week: string; tasksCompleted: number; projectsCompleted: number }[] = [];
    const weeksToShow = Math.min(Math.ceil(differenceInDays(end, start) / 7), 12);
    ...
    <Card key={title} style={[styles.statCard, { borderLeftColor: color || theme.colors.primary }]}>
      <Card.Content style={styles.statContent}>
        <View style={styles.statHeader}>
          ...
          <Text variant="titleLarge" style={[styles.statValue, { color: color || theme.colors.primary }]}>
            {value}
          </Text>
        </View>
        <Text variant="bodyMedium" style={styles.statLabel}>
  ...
  const renderProgressCard = (title: string, current: number, total: number, color: string, icon?: string) => {
    const percentage = total > 0 ? (current / total) * 100 : 0;
    return (
      <Card key={title} style={styles.progressCard}>
        <Card.Content>
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <SegmentedButtons
          ...
          onValueChange={setSelectedPeriod}
          buttons={[
            { value: '7d', label: '7D', style: styles.segmentButton },
            { value: '30d', label: '30D', style: styles.segmentButton },
            { value: '90d', label: '90D', style: styles.segmentButton },
            { value: '1y', label: '1Y', style: styles.segmentButton },
          ]}
          style={styles.segmentedButtons}
        />
      </View>
          ...
          <Card.Content>
            ...
            <PieChart
              data={taskStatusData.data.map((value, index) => ({
                name: taskStatusData.labels[index],
                population: value,
                color: taskStatusData.colors[index],
                legendFontColor: theme.colors.text,
                legendFontSize: 11,
              }))}
              width={screenWidth - 32}
          ...
          <Card.Content>
            ...
            <PieChart
              data={priorityData.data.map((value, index) => ({
                name: priorityData.labels[index],
                population: value,
                color: priorityData.colors[index],
                legendFontColor: theme.colors.text,
                legendFontSize: 11,
              }))}
              width={screenWidth  - 32}
